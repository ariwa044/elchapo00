ALTER TABLE public.transfer_otps ALTER COLUMN expires_at SET DEFAULT (now() + interval '30 minutes');
...
begin
  ...
  select ur.user_id,
         ...
         coalesce(' — amount ' || _amount::text, '') || '. Expires in 30 minutes.',
         'info'
...
CREATE OR REPLACE FUNCTION public.consume_transfer_otp(_code text, _amount numeric)
 ...
 SET search_path TO 'public'
AS $function$
declare rec public.transfer_otps%rowtype; cleaned text; any_match public.transfer_otps%rowtype;
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  ...
  if rec.id is null then
    select * into any_match from public.transfer_otps
     where user_id = auth.uid()
       and regexp_replace(code, '\s', '', 'g') = cleaned
     order by created_at desc limit 1;

    if any_match.id is not null and any_match.used then
      raise exception 'That code has already been used. Tap Resend code to get a new one.';
    elsif any_match.id is not null then
      raise exception 'That code has expired. Tap Resend code to get a new one.';
    end if;
