-- Transfers now require the 4-digit payment PIN (verified server-side)
drop function if exists public.send_money(text, numeric, text, text, text);
...
begin
  ...
  -- 4-digit payment PIN is mandatory
  perform public.assert_payment_pin(_pin);

  if _otp_code is not null then
    otp_result := public.verify_otp(auth.uid(), _otp_code, null);
    if not (otp_result->>'success')::boolean then raise exception 'Invalid or expired OTP'; end if;
  end if;
...
revoke all on function public.send_money(text, numeric, text, text, text, text) from anon, public;
grant execute on function public.send_money(text, numeric, text, text, text, text) to authenticated;

drop function if exists public.bank_transfer(numeric, numeric, text, text, text, text, text, text, text, text, text, text, text);
...
create or replace function public.bank_transfer(
  ...
  _iban text default null, _currency text default null, _purpose text default null,
  _kind text default 'local', _otp_code text default null, _pin text default null
) returns jsonb language plpgsql security definer set search_path = public as $$
declare
  sender public.profiles%rowtype;
  txid uuid;
...
revoke all on function public.bank_transfer(numeric, numeric, text, text, text, text, text, text, text, text, text, text, text, text) from anon, public;
grant execute on function public.bank_transfer(numeric, numeric, text, text, text, text, text, text, text, text, text, text, text, text) to authenticated;
